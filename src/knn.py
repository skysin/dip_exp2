...
class KNN(object):
    ...
    def load_testing_data(self):
        ...
        for i in range(500):
            ...
            for k in range(7):
                #print(k)
                tp = [self.label_set[int(j)] for j in self.knn_list[k].kneighbors(source[k], n_neighbors=4, return_distance=False)[0]]
                tar.extend(tp)
            ...
            print("accuracy: %d, %d, %f" % (count, (i + 1), float(count / (i + 1))))
    ...
    def build_model(self):
        for i in range(7):
            ...
            #self.knn_list[i].fit(newdata, self.label_set)
            #self.knn_list[i].partial_fit(self.training_data[i + 1], self.label_set)
            #print(len(self.training_data[i + 1]))

            #for j in range(len(self.training_data[i + 1])):
                #print(j, self.training_data[i + 1][j].size)
            try:
